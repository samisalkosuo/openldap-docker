...
```
docker run -d -p 389:1389 -p 636:1636 --name openldap-demo -v /etc/letsencrypt/live/openldap.example.com/privkey.pem:/certs/ldap.key -v /etc/letsencrypt/live/openldap.example.com/cert.pem:/certs/ldap.crt -v /etc/letsencrypt/live/openldap.example.com/chain.pem:/certs/ca.crt kazhar/openldap-demo
```
